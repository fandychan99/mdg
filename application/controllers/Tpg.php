...
class Tpg extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model(array('M_User', 'M_Role', 'M_Sekolah', 'M_Guru', 'M_Tpg'));
    ...
    public function index()
    {
        ...
        // var_dump($guru);
        $this->template->display('tpg/v_index', $data);
    }

    public function ajax_list()
    {
        $list = $this->M_Tpg->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_Tpg->count_all(),
            "recordsFiltered" => $this->M_Tpg->count_filtered(),
            "data" => $data,
        );
    ...
    public function save(){
        try {
            ...
            $config['upload_path'] = './uploads/ptg'; // Tentukan direktori penyimpanan gambar
            $config['allowed_types'] = 'pdf'; // Jenis file yang diizinkan
            $config['max_size'] = 2048; // Batasan ukuran file (dalam kilobyte)
            // $new_name =   
            $config['file_name'] = "ptg-" . bin2hex(random_bytes(24));
            ...
            $data = $this->M_Tpg->save($data_simpan);
    ...
    public function get_byid(){
        ...
        $data["header"] = $this->M_Tpg->get_data_by_id($id);
