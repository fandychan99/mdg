...
class Honda extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model(array('M_User', 'M_Role', 'M_Sekolah', 'M_Guru', 'M_Honda'));
    ...
    public function index()
    {
        ...
        // var_dump($guru);
        $this->template->display('honda/v_index', $data);
    }

    public function ajax_list()
    {
        $list = $this->M_Honda->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_Honda->count_all(),
            "recordsFiltered" => $this->M_Honda->count_filtered(),
            "data" => $data,
        );
    ...
    public function save(){
        try {
            ...
            $config['upload_path'] = './uploads/honda'; // Tentukan direktori penyimpanan gambar
            $config['allowed_types'] = 'pdf'; // Jenis file yang diizinkan
            $config['max_size'] = 2048; // Batasan ukuran file (dalam kilobyte)
            // $new_name =   
            $config['file_name'] = "honda-" . bin2hex(random_bytes(24));
    ...
    public function get_byid(){
        ...
        $data["header"] = $this->M_Honda->get_data_by_id($id);
    ...
    public function delete(){
        $id = decrypt_url($this->input->get("id"));
        // echo $id;
        $hasil = $this->M_Honda->delete_id($id);
