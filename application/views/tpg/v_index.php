            ...
            <div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
                    <li class="breadcrumb-item text-muted">
                        <a href="#" class="text-muted text-hover-primary">PENGAJUAN TPG </a>
                    </li>
...
<div class="card mb-5 mb-xl-8">
    <div class="card-header border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bold fs-3 mb-1">LIST TPG</span>
        </h3>
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header" id="kt_modal_add_user_header">
                <h2 class="fw-bold">PENGAJUAN TPG</h2>
                <div class="btn btn-icon btn-sm btn-active-light-primary ms-2" data-bs-dismiss="modal" aria-label="Close">
                    <i class="ki-duotone ki-cross fs-1"><span class="path1"></span><span class="path2"></span></i>
                </div>
            </div>
            <div class="modal-body px-5 my-7">
                <form id="kt_modal_add_user_form" class="form" action="<?= base_url(); ?>tpg/save" method="POST">
...
<script>
    ...
    $(document).ready(function() {
        ...
        table = $('#kt_datatable_fixed_header').DataTable({
            ...
            "ajax": {
                "url": "<?php echo base_url('tpg/ajax_list') ?>",
                "type": "POST",
                "data": function(d) {
...
<script>
    function edit_data(id) {
        cleardata();
        $url = "<?= base_url() ?>tpg/get_byid?id=" + id;
        console.log($url);
        $.ajax({
            url: $url,
            type: "GET",
    ...
    function delete_data($id, $produk) {
        $url = "<?= base_url() ?>tpg/delete?id=" + $id;
        Swal.fire({
            title: 'Apakah Anda yakin menghapus Pengajuan ' + $produk + ' ?',
